...
//g_LocalPlayer->RPC_Client_ChangeRectiftAvatar(EAvatarSlotType::EAvatarSlotType__EAvatarSlotType_ClothesEquipemtSlot,1405871,true);
...
//ImGui::Checkbox("CS",&CS); 
//ImGui::SliderFloat("速度", &sd, 0.0f, 15.0f);
ImGui::Checkbox("CS",&CS); 
ImGui::Separator();
...
ImGui::RadioButton("身体", &Aim,2);
        ImGui::SameLine();
        ImGui::RadioButton("随机(函数追踪)", &Aim,3);
ImGui::Text("打击方式:");        
...
ImGui::Button("人物自杀");
ImGui::SameLine();
if(ImGui::Button("丢全物品")){
g_LocalPlayer->STPlayerController->DropAllItem();
}
...
if(v加速){
ImGui::SliderFloat("速度", &Qo, 1.0f, 15.0f);
}
ImGui::Checkbox("超广角", &WideView);
                        if (WideView){
                         ImGui::SliderInt("视野范围", &WideValue, 90, 140);
}
if(ImGui::Checkbox("世界速度",&SJJS)){
g_LocalPlayer->CustomTimeDilation = Sd;
							
} 
if(SJJS){
ImGui::SliderFloat("速度", &sd, 0.0f, 5.0f);
}
if(ImGui::Checkbox("人物高度",&RWGD)){
g_LocalPlayer->CrouchHalfHeight=GD;
} 
if(RWGD){
ImGui::SliderFloat("人物高度", &GD, -30.0f, 100.0f);
}
...
if(Tp){
ImGui::SliderFloat("速度", &KL, 0.0f, 20.0f);
}
}
...
ImGui::Separator();
if(ImGui::Button("Save the current configuration")){SavePreferences();
}
ImGui::SameLine();
if(ImGui::Button("Load configuration")){LoadPreferences();
}
...
if(Tp){
ImGui::SliderFloat("Vable", &KL, 0.0f, 15.0f);
}
}
